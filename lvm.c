...
#define doreturn()                                     \
  do {                                                 \
    if (ci->callstatus & CIST_FRESH)                   \
      vmframe_end(L, ci); /* end this frame */         \
    else { /* continue running caller in this frame */ \
      vmframe_return(L, ci->previous);                 \
    }                                                  \
  } while (0)
...
#define vmframe_start(L, nci) ci = (nci); goto startfunc
#define vmframe_return(L, nci) ci = (nci); goto returning
#define vmframe_end(L, ci) return
