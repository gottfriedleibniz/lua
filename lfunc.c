...
static void calldefermethod (lua_State *L, TValue *func, TValue *err) {
  if (l_likely(ttisfunction(func))) {
    ...
    L->top.p = top + 2;  /* add function and arguments */
    luaD_callnoyield(L, top, 0);
  }
}
#endif
...
*/
#if defined(LUA_EXT_DEFER)
void luaF_newtbcupval_ext (lua_State *L, StkId level, int deferred) {
  ...
  while (cast_uint(level - L->tbclist.p) > MAXDELTA) {
    ...
    L->tbclist.p->tbclist.deferred = 0;
    L->tbclist.p->tbclist.delta = 0;
  }
  level->tbclist.deferred = deferred ? 1 : 0;
